...
import { Copy, Delete, Download, DownloadIcon, Trash } from 'lucide-react'
  ...
  return (
    <div className='flex flex-col md:flex-row gap-5 border p-4 bg-gray-900 rounded-lg'>
      ...
      <Link to={`/link/${url?.id}`} className='flex flex-col flex-1'>
        <span>{url?.title}</span>
        <span className='text-3xl text-blue-400 font-bold  hover:underline cursor-pointer'>{url?.custom_url? url?.custom_url: url?.short_url}</span>
      ...
      <div className='flex gap-2'>
         <Button className='cursor-pointer' variant="ghost" onClick={()=> navigator.clipboard.writeText(`https://trimrr.in/${url?.short_url}`)}>
            <Copy/>
         </Button>
         <Button className='cursor-pointer'  variant="ghost" onClick={downloadImage}>
            <Download/>
         </Button>
         <Button className='cursor-pointer'  variant="ghost" onClick={()=>fnDelete().then(()=>fetchUrls())}>
